...
<template>
	<div class="apply">
		...
		<tk-container style="bottom:50px;">
			...
			style="position:fixed;bottom:50px;height:inherit;width:100%;top:100px;"
			v-model="data" 
			:param="{
		...
		<div class="apply-button">
			...
			<div v-if="!hasChoosedNum" @click="choose_from_local" class="yixuan">
				从历史纪录中导入数据
			</div>
		...
		computed: {
			...
			hasChoosedJson() {
				...
				for(var key in this.hasChoose) {
					arr.push({
						goodsModelId: this.hasChoose[key].goodsModelId,
						goodsId:key,
						num: this.hasChoose[key].count,
						identifier: this.hasChoose[key].identifier
					})
				}
				return arr
			},
