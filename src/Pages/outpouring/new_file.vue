				...
				<pot-slide ref="slide" :loop="false" :initial-index="initialIndex" :auto-play="false" :show-dots="false" :options="{
					...
					<pot-slide-item>
						<tk-scroll :options="{
							pullDown:true,
							pullUp:true
						}" :style="{height:innerHeight-100+'px'}" class="outpouring-scroll" v-model="list" :param="{
							uid:$store.state.uid,
							status:0
						}" url="cs/getOutpouringList">
							<tk-cell-four :key=index @click="$router.push({
					...
					<pot-slide-item>
						<tk-scroll :options="{
							pullDown:true,
							pullUp:true
						}" :style="{height:innerHeight-100+'px'}" class="outpouring-scroll" v-model="list_2" :param="{
							uid:$store.state.uid,
							status:12
						}" url="cs/getOutpouringList">
							<tk-cell-four :key=index @click="$router.push({
					...
					<pot-slide-item>
						<tk-scroll :options="{
							pullDown:true,
							pullUp:true
						}" :style="{height:innerHeight-100+'px'}" class="outpouring-scroll" v-model="list_3" :param="{
							uid:$store.state.uid
						}" url="cs/getOutpouringList">
							<tk-cell-four :key=index @click="$router.push({
								path:'/outpouring_detail',
...
<script>
	export default{
		...
		methods:{
			ajax(type){
				var page=['page','page_2','page_3'][type]
				var canpullUp=['canpullUp','canpullUpb','canpullUpc'][type]
				this.$http.post('cs/getOutpouringList',{
					uid:this.$store.state.uid,
					status:['0','12'][type],
					page:this[page]
				}).then(d=>{
					var list=['list','list_2','list_3'][type];
					if(this[page]==1){
						this[list]=d;
					}else{
						if(d.length==0){
//							this.$refs.scroll.forceUpdate(true);
			...
			changePage(current) {
//				this.$store.state.eventPage=current;
